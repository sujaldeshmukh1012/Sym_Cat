...
def load_report_fields(inspection_id: str) -> dict[str, str]:
    ...
    inspection_row = fetch_row(
        client,
        table_candidates=["inspection", "inspections"],
        columns="id, customer_name, inspector, fleet_serial",
        filter_column="id",
        filter_value=inspection_id,
    )
    ...
    return {
        ...
        "model": fleet_row.get("model") or DEFAULT_REPORT_FIELDS["model"],
        "inspector": inspection_row.get("inspector") or DEFAULT_REPORT_FIELDS["inspector"],
        "date": format_date(report_row.get("created_at")),
    }
