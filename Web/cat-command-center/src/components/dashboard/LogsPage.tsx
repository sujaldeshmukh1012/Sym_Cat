...
export default function LogsPage() {
  ...
  async function fetchLogs() {
    ...
    const [tasksRes, orderCartRes] = await Promise.all([
      supabase.from('task').select('id, fleet_serial, inspection_id, title, state, description, feedback, created_at').order('created_at', { ascending: false }),
    ...
    if (tasksRes.error) {
      ...
      const latestOrderStatusByInspectionId: Record<string, string> = {};
      ...
      const merged = (tasksRes.data || []).map((task: any) => {
        const inspectionKey = String(task.inspection_id ?? '');
        return { ...task, resolved_state: latestOrderStatusByInspectionId[inspectionKey] || task.state };
      });
  ...
  function getStateBadgeClass(stateValue: string | null) {
    const normalized = String(stateValue ?? '').toLowerCase().trim();
    if (['completed', 'confirmed', 'approved'].includes(normalized)) return 'badge-success';
    if (['pending', 'in_progress', 'in progress'].includes(normalized)) return 'badge-warning';
    if (['declined', 'rejected', 'failed'].includes(normalized)) return 'badge-critical';
    return 'badge-info';
  }
      ...
      <div className="card">
        <div className="card-header">
          <div className="search-bar">
            ...
            <input placeholder="Search by id, fleet, inspection, title, state, description, or feedback…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
